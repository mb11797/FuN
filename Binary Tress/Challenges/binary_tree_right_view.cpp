...
void right_view_binary_tree(node*root, int cur_level, int *max_visited){
    if(root == NULL)
        return;
    ...
    right_view_binary_tree(root->right, cur_level + 1, max_visited);
    right_view_binary_tree(root->left, cur_level + 1, max_visited);
}
...
//    right_view_bt(root);
    ...
    int max_visited = 0;
    right_view_binary_tree(root, 1, &max_visited);
